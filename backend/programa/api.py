...
def programa_cuestionarios_y_respuestas(request, pk):
    ...
    try:
        ...
        # Procesar cuestionario pre si existe
        if cuestionario_pre:
            # Filtrar preguntas que no sean de tipo likert
            preguntas_pre = []
            indices_preguntas = []  # Para mantener el índice de las preguntas no likert
            ...
            for respuesta in respuestas_pre:
                ...
                # Convertir el diccionario de respuestas en una lista ordenada, excluyendo las likert
                respuestas_lista = []
                for i, pregunta in enumerate(cuestionario_pre.preguntas):
                    if i in indices_preguntas:  # Solo incluir respuestas de preguntas no likert
        ...
        # Procesar cuestionario post si existe
        if cuestionario_post:
            # Filtrar preguntas que no sean de tipo likert
            preguntas_post = []
            indices_preguntas = []  # Para mantener el índice de las preguntas no likert
